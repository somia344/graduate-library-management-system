        ...
        <div class="dashboard-main">
            ...
            <div class="top-bar">
                <div class="page-title">
                    <h2 class="fw-bold" style="color: #0D5C63; margin: 0;">Edit Book</h2>
                    <p class="text-muted mt-1" style="color: #6B7280;">Update book information</p>
                </div>
            ...
            <div class="form-card">
                <div class="card-header-custom">
                    <h5><i class="fas fa-edit me-2" style="color: #0D5C63;"></i> Book Information</h5>
                </div>
                    ...
                    <form action="{{ route('librarian.books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" class="form-control-custom" required value="{{ old('title', $book->title) }}" placeholder="Enter book title">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="author" class="form-control-custom" required value="{{ old('author', $book->author) }}" placeholder="Enter author name">
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <select name="category" class="form-control-custom" required>
                                    ...
                                    <option value="Music" {{ old('category', $book->category) == 'Music' ? 'selected' : '' }}>Music</option>
                                    <option value="Philosophy" {{ old('category', $book->category) == 'Philosophy' ? 'selected' : '' }}>Philosophy</option>
                                    <option value="Psychology" {{ old('category', $book->category) == 'Psychology' ? 'selected' : '' }}>Psychology</option>
                                    <option value="Business" {{ old('category', $book->category) == 'Business' ? 'selected' : '' }}>Business</option>
                                    <option value="Economics" {{ old('category', $book->category) == 'Economics' ? 'selected' : '' }}>Economics</option>
                                    <option value="Law" {{ old('category', $book->category) == 'Law' ? 'selected' : '' }}>Law</option>
                                    <option value="Medicine" {{ old('category', $book->category) == 'Medicine' ? 'selected' : '' }}>Medicine</option>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="isbn" class="form-control-custom" value="{{ old('isbn', $book->isbn) }}" placeholder="Enter ISBN number">
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" name="quantity" class="form-control-custom" required min="1" value="{{ old('quantity', $book->quantity) }}" id="quantity" placeholder="Enter quantity">
                                <small class="text-muted">Current available: {{ $book->available }} copies</small>
                            </div>
                                ...
                                <div class="file-upload-wrapper">
                                    @if($book->book_image)
                                        <div class="current-image">
                                            ...
                                            <img src="{{ asset($book->book_image) }}" alt="{{ $book->title }}" class="image-preview-current">
                                        </div>
                                    @endif
                                    ...
                                    <small class="text-muted d-block mt-2">Leave empty to keep current image. Max size: 2MB</small>
                                    <div id="imagePreview" class="image-preview mt-3" style="display: none;">
                                        <label>New Image Preview:</label>
                                        <img id="previewImg" src="#" alt="Preview" style="max-width: 150px; border-radius: 10px; margin-top: 10px;">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="info-note">
                            ...
                            <strong>Note:</strong> Changing the quantity will affect available copies. If you decrease quantity, ensure it doesn't go below currently issued copies ({{ $book->quantity - $book->available }} issued).
                        </div>
...
document.getElementById('quantity').addEventListener('change', function() {
    const issuedCount = {{ $book->quantity - $book->available }};
    const newQuantity = parseInt(this.value);
    
    if (newQuantity < issuedCount) {
        alert(`Warning: ${issuedCount} copies are currently issued. Quantity cannot be less than issued copies.`);
        this.value = issuedCount;
    }
});
</script>
@endsection